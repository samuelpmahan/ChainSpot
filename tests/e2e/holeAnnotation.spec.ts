...
test('hole annotation: place tee/basket/shots/bends via the radial menu, correct with remove-last controls, no cross-contamination between holes', async ({
	...
	await placePoint(page, box.x + 300, box.y + 250, 'basket');
...
test('hole annotation: existing tee, basket, shot, and bend markers drag without placement duplication; a non-dragging click opens the delete menu instead of placing', async ({
	...
	await expect(page.getByTestId('radial-action-delete')).toBeVisible();
...
test('hole annotation: corridor width applies to every hole (UDisc parity), and a new hole inherits the width in use', async ({
	...
	await placePoint(page, box2.x + 450, box2.y + 300, 'tee');
	await placePoint(page, box2.x + 550, box2.y + 400, 'basket');
