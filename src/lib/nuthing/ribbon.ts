...
function areaResizeRgb(image: RgbaImage, ew: number, eh: number): Float32Array {
	...
	for (let y = 0; y < h; y++) {
		...
		if (interiorRow) {
			...
			for (; x < xHi; x++, o += 3, p0 += 3, p1 += 3) {
				...
				out[o + 1] = img[p0 + 1] * w00 + img[p0 + 4] * w01 + img[p1 + 1] * w10 + img[p1 + 4] * w11;
				out[o + 2] = img[p0 + 2] * w00 + img[p0 + 5] * w01 + img[p1 + 2] * w10 + img[p1 + 5] * w11;
			}
...
export function computeRibbonSupport(image: RgbaImage, options: RibbonOptions = {}): SupportField {
	...
	const p995 =
		nonzero.length > 0
			? nonzero[Math.min(nonzero.length - 1, Math.floor(0.995 * (nonzero.length - 1)))]
			: 1;
...
export function scoreEndpointComponents(
	...
	const tiers: [
		(c: EndpointComponentCandidate) => boolean,
		(c: EndpointComponentCandidate) => boolean
	][] = [
		...
		for (let k = 1; k < run.length; k++) {
			...
			for (let c = 0; c < cur.length; c++) {
				...
				for (let p = 0; p < prev.length; p++) {
					const d = Math.hypot(prevTriples[p].b.x - cur[c].t.x, prevTriples[p].b.y - cur[c].t.y);
