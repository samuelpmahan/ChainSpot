...
{#if tick && operation}
	<article class="tick-inspection" data-pcr={pcr.id} data-tick={tick.opId}>
		...
		<dl class="identity-strip">
			<div><dt>Run Args identity</dt><dd><code>{pcr.paramsHash ?? 'UNKNOWN'}</code></dd></div>
			<div><dt>Plan</dt><dd><code>{pcr.planFingerprint}</code></dd></div>
			<div><dt>Run result</dt><dd><code>{pcr.runResultId}</code></dd></div>
		</dl>
		...
		<div class="flow">
			...
			<section>
				...
				{#if missingOutputs.length}<p class="warning">Declared but unwritten: {missingOutputs.join(', ')}</p>{/if}
			</section>
		</div>
		...
		<section class="materializations">
			...
			{:else}
				<p class="unknown">No persisted Materialization at this boundary. PxC outputs remain inspectable by address.</p>
			{/if}
		</section>
...
<style>
	.tick-inspection { display: grid; gap: 1rem; color: #171717; font: 14px/1.4 ui-sans-serif, system-ui, sans-serif; }
	header { display: flex; justify-content: space-between; align-items: end; gap: 1rem; }
	header span, h3, dt { color: #6d28d9; font-size: .72rem; font-weight: 750; letter-spacing: .06em; text-transform: uppercase; }
	h2 { margin: .15rem 0 0; font: 700 1.2rem/1.2 ui-monospace, monospace; }
	code { overflow-wrap: anywhere; }
	.identity-strip { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: .5rem; margin: 0; }
	.identity-strip div, section { border: 1px solid #d4d4d4; background: #fafafa; padding: .75rem; }
	dt { margin-bottom: .25rem; }
	dd { margin: 0; font-size: .72rem; }
	.flow { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1.35fr) minmax(0, 1fr); gap: .75rem; }
	h3 { margin: 0 0 .45rem; }
	ul { margin: 0; padding-left: 1.1rem; }
	li + li { margin-top: .35rem; }
	.calculation li, .materializations li { display: grid; gap: .15rem; }
	small { color: #737373; overflow-wrap: anywhere; }
	.unknown { color: #737373; }
	.warning, .replacement { color: #a21caf; font-weight: 700; }
	@media (max-width: 780px) { .flow, .identity-strip { grid-template-columns: 1fr; } }
</style>
