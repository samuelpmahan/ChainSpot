<script lang="ts">
  ...
  let { pxc, run, width, height, source, requestedSelection }: {
    pxc: PxC; run: PqlRun; width: number; height: number; source: string;
    requestedSelection?: { calculation: number; item: string; part?: string };
  } = $props();
  ...
  $effect(() => {
    const request = requestedSelection;
    if (request) untrack(() => {
      update({ calculation: request.calculation, item: request.item, selectedPart: request.part ?? null, hidden: [] });
      filter = ''; pixelMessage = '';
    });
  });
