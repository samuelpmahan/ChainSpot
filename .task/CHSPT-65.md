...
- **Key regression test — completion handoff, course-length-aware.** (Revised per independent review: the pointer-placement helpers in `annotateCourseSidebar.test.ts` are pre-existing red, so the new regressions must not sit on that path.) A dedicated `annotateCourseCompletionHandoff.test.ts` seeds already-placed holes directly through the injected editor (the same seeding a reopened draft uses), confirms them through the sidebar's plain DOM buttons, and asserts exactly what CHSPT-65 changed: complete course → completion panel → Continue → pending `AnnotatedRound` → `/create-graphics`, at both 18/18 and 9/9, with Save course to memory and Map a round on this course present and unchanged, plus the guard that confirmed work beyond the configured length blocks a contradictory short-course completion. Repo reality: arbitrary-hole completion has NOT landed — the guided sidebar hard-coded 18, so a 9-hole course could never complete; a minimal 9-or-18 course-length choice was added per the approved revision. Additionally, the thrown-round slot now has a workflow lifecycle (review finding #1): a course-source intake in Annotate Course attaches the kept round to that course, and a second intake while attached clears it as stale — unit-covered in `thrownRoundFlow.test.ts` and browser-verified against the reviewer's exact repro.
