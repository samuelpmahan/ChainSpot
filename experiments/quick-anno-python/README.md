...
Purpose: test whether local Python can be the cheap investigation/authoring surface while preserving the same compositional PCR shape used by the S0/S1 Mermaid proof.

This is intentionally not a second ChainSpot executor. Python authors a graph; ChainSpot/PxC remains the intended execution owner.
...
PYTHONPATH=packages/quick_anno_py python3 -m unittest discover -s packages/quick_anno_py/tests -v
```
