...
## Walking-path pixel classification: benchmarked, looks easy (2026-08-12)

Separate signal from the paused ribbon work above — the played-round purple walking path is a
hard-edged, near-opaque two-tone stroke, and a single-fixture benchmark
(`walking_path_benchmark.py`, annotation fixture in `resources/walking-path/`, generated by
`walking_path_make_annotation.py`) shows it is trivially separable with cheap per-pixel features:
F1 0.999 on held-out spatial regions of `resources/ThrownRounds/IMG_5612.png`, and a
close/open/skeletonize pass recovers an essentially connected path skeleton. Results, ranking,
diagnostic contact sheets and the recommended next implementation are in
`walking-path-findings.md` + `walking-path-results/`. Single fixture only — no generalization
claim.
