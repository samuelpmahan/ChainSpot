...
export const HELP_CATALOG = Object.freeze([
  ...
  record('sweep/batch', {
    ...
    forms: ['lab sweep batch [--through GATE] CONFIG.json all', 'lab sweep batch [--through GATE] CONFIG.json dev', 'lab sweep batch [--through GATE] CONFIG.json demo', 'lab sweep batch [--through GATE] CONFIG.json COURSE...'],
    options: [option('--through', 'Execute the dependency-complete plan slice for every selected case.', { value: 'G1|G2|G3|G4|G5|G6|G7', defaultValue: 'G3', constraints: 'Optional; every gate G1-G7 is a valid batch cutoff.' })],
  ...
  record('gate-vocabulary', { title: 'GATE VOCABULARY', summary: 'LAB knowledge and execution use one canonical gate order; shared-set is infrastructure ownership, not a scheduled gate.', forms: ['LAB knowledge catalog and Engine execution share one canonical order: G0 Intake, G1 Badges, G2 Baskets, G3 Visible Tees, G4 Recovery (Tee + Basket), G5 Straight Test, G6 Assignment, G7 Bend Refinement'], caveats: ['`lab gates 3` queries the canonical G3 knowledge card; it is not an instruction to execute `--through G3`.', '`lab sweep --through` accepts every gate G1-G7; each cutoff runs the dependency-complete chronological prefix through that gate and the receipt names later-gate prerequisites and unscheduled operations. `shared-set` is cross-gate infrastructure ownership, not a scheduled gate.'] }),
...
export const EXECUTABLE_SWEEP_GATES = Object.freeze(['G1', 'G2', 'G3', 'G4', 'G5', 'G6', 'G7']);
