...
export const HELP_CATALOG = Object.freeze([
  ...
  record('sweep', {
    ...
    options: [option('--through', 'Execute the dependency-valid plan slice through this gate.', { value: 'G1|G2|G3|G4|ST|G5', constraints: 'shared is cross-gate infrastructure and is rejected; there is no --stop-after' })],
    ...
    outputs: ['Canonicalization report, gate timeline, operation receipts, and artifacts.'],
  ...
  record('sweep/through', { parent: 'sweep', title: 'SWEEP THROUGH — dependency-valid slice', summary: 'Execute from the plan start through an executable named gate.', forms: ['lab sweep --through G1|G2|G3|G4|ST|G5 CONFIG.json INPUT... [TRUTH.json]'], availability: AVAILABILITY.CLI_ONLY, caveats: ['`shared` is vocabulary for cross-gate infrastructure and is rejected. There is no --stop-after.'] }),
  ...
  record('gate-vocabulary', { title: 'GATE VOCABULARY', summary: 'LAB knowledge gates and executable engine gates are two different vocabularies.', forms: ['LAB knowledge catalog: lab gates 0..7 — 0 Crop+Stitch, 1 Badges, 2 Baskets, 3 Tees, 4 Endpoint Recovery, 5 Straight Test, 6 Assignment, 7 Bend Refinement', 'Engine execution: G1 Badges, G2 Baskets, G3 Tees, G4 Tee→Badge, ST Straight Test, G5 Path, shared (cross-gate infrastructure)'], caveats: ['`lab gates 3` queries the numeric knowledge card; it is not an instruction to execute `--through G3`.', '`lab sweep --through` accepts G1, G2, G3, G4, ST, or G5. `shared` is valid engine vocabulary but not a legal execution cutoff.'] }),
...
export const EXECUTABLE_SWEEP_GATES = Object.freeze(['G1', 'G2', 'G3', 'G4', 'ST', 'G5']);
