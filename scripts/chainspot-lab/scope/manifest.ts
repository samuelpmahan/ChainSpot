...
function point(value: unknown, where: string): PointTuple {
	if (!Array.isArray(value) || value.length !== 2 || !value.every(isFiniteNumber)) throw new Error(`lab scope: ${where} must be [x,y].`);
	return [value[0], value[1]];
}
function box(value: unknown, where: string): BoxTuple {
	if (!Array.isArray(value) || value.length !== 4 || !value.every(isFiniteNumber) || value[2] <= 0 || value[3] <= 0) throw new Error(`lab scope: ${where} must be [x,y,w,h] with positive w/h.`);
...
function parseRequest(value: unknown, where: string): ScopeRequest {
	...
	if (raw.full !== undefined && raw.full !== true) throw new Error(`lab scope: ${where}.full must be true when present.`);
	const request: ScopeRequest = {
		...
		full: raw.full === true ? true : undefined,
	...
	const kinds = [request.point, request.box, request.mark, request.dots, request.path, request.hole, request.full].filter((v) => v !== undefined).length;
	if (kinds !== 1) throw new Error(`lab scope: ${where} must specify exactly one of point, box, mark, dots, path, hole, or full.`);
	return request;
}
function parseCase(value: unknown, where: string): ScopeManifestCase {
	...
	return { name: raw.name, image: raw.image, annotation: typeof raw.annotation === 'string' ? raw.annotation : undefined, scopes: raw.scopes.map((s, i) => parseRequest(s, `${where}.scopes[${i}]`)) };
}
export interface LoadedScopeManifest { readonly path: string; readonly dir: string; readonly cases: readonly ScopeManifestCase[]; }
export function loadScopeManifest(manifestPath: string): LoadedScopeManifest {
	const path = resolve(manifestPath); let raw: unknown;
	try { raw = JSON.parse(readFileSync(path, 'utf8')); } catch (err) { throw new Error(`lab scope: could not read/parse manifest at ${path}: ${(err as Error).message}`); }
	...
	if (Array.isArray(manifest.cases)) cases = manifest.cases.map((c, i) => parseCase(c, `cases[${i}]`));
	else {
		if (typeof manifest.image !== 'string' || !Array.isArray(manifest.scopes) || manifest.scopes.length === 0) throw new Error('lab scope: manifest needs either cases[] or top-level image + scopes[].');
		cases = [{ name: 'default', image: manifest.image, annotation: typeof manifest.annotation === 'string' ? manifest.annotation : undefined, scopes: manifest.scopes.map((s, i) => parseRequest(s, `scopes[${i}]`)) }];
	}
...
export function resolveManifestCasePaths(baseDir: string, entry: ScopeManifestCase): ScopeManifestCase {
	return { ...entry, image: resolve(baseDir, entry.image), annotation: entry.annotation ? resolve(baseDir, entry.annotation) : undefined };
}
