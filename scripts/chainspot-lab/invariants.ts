...
export const INVARIANT_CARDS: readonly InvariantCard[] = [
	{
		...
		claim:
			'UI chrome does not exist downstream; every measurement, annotation, render, and gate uses the flattened crop-local raster.',
		scope: 'All ChainSpot LAB runs.',
		evidence: [
			'The source-row/source-tile transforms are sufficient provenance for returning to inputs.',
			'A missed viewport.top subtraction previously displaced an entire overlay.'
		],
		...
		breakers: [
			'Subtracting the crop offset zero or two times.',
			'Measuring a source tile after the canonical composite exists.'
		],
		retest:
			'Hash and dimension the canonical raster, then project known landmarks through every recorded source transform.',
	...
	},
	{
		...
		claim:
			'Observed order is badges > basket sprites > basket rings/fills > corridor > tee pads > walking path > satellite imagery.',
		scope: 'Measured UDisc renderer family represented by Dev72.',
		evidence: [
			'Corridor paint covers tee borders.',
			'Sprites and badges occlude lower layers rather than being displaced by them.'
		],
		...
		breakers: [
			'A UDisc renderer update.',
			'A screenshot produced by a different map/overlay mode.'
		],
	...
	},
	{
		...
		claim:
			'The route is a constant-width polyline-like graphic with straight arms, constructed bend joins, and semicircular endpoint caps.',
		scope:
			'Dev72 renderer evidence; Alex miter observations are transfer precedent, not a universal join law.',
		evidence: [
			'Historical cap probes place cap centers about 4.5 px inside endpoints with radius W/2.',
			'Alex visual review supported mitered inner/outer bend corners.'
		],
		use: [
			'Fit sustained arms and paired edges; treat endpoint caps and bend joins as different geometry.'
		],
		doNotInfer: [
			'A centerline bend vertex is not either visible miter corner.',
			'An isolated edge-like pixel is not a corridor arm.'
		],
		breakers: [
			'Antialiasing and alpha blending over terrain.',
			'A renderer using rounded joins or variable width.'
		],
		retest:
			'Measure both rails and endpoint profiles on a new course before transferring join/cap assumptions.',
	...
	},
	{
		...
		claim:
			'Within each measured course, corridor width is approximately constant even though it differs between courses.',
		scope:
			'Dev72: annotated widths 40, 30, 37, and 37 px; unseen courses require fresh measurement.',
		evidence: [
			'Full-width-at-half-maximum estimates broadly agree with annotation-derived widths.'
		],
		use: [
			'Set paired-edge separation, cap radius, masks, and bend-capsule width from one course measurement.'
		],
		doNotInfer: [
			'Do not reuse a Dash width on NorthPark.',
			'A single local width sample may be contaminated.'
		],
		breakers: [
			'Mixed zoom screenshots.',
			'Perspective/resampling or a renderer with width varying by hole.'
		],
		retest:
			'Measure several clean perpendicular cross-sections and report their spread before choosing W.',
	...
	},
	{
		...
		detectors: ['D05-occluded-tee-recovery', 'D06-ribbon-support', 'D12-capsule-bends'],
		claim:
			'The same corridor color composites differently over local ground, so absolute brightness is not stable evidence.',
		scope: 'Measured overview captures; historical estimates C≈RGB[150,155,145], alpha≈0.61–0.90.',
		evidence: [
			'Corridor over dark trees measured far darker than corridor over open ground.',
			'Tree underlay caused both detector and annotation centerline bias.'
		],
		...
		breakers: [
			'Contaminated background-reference windows.',
			'Terrain edges aligned with the corridor.'
		],
	...
	},
	{
		...
		claim:
			'The measured basket family uses a 42×66 sprite; the semantic endpoint is the pole tip, approximately sprite bottom +4 px, not the sprite center.',
		scope: 'Dev72 UDisc basket sprite family.',
		evidence: [
			'Occlusion-tolerant template matching reached 72/72 Dev72 candidate recall.',
			'Instances show small compression variation rather than byte identity.'
		],
		...
		breakers: [
			'Different sprite scale/theme.',
			'Cropping or compression that changes apparent bounds.'
		],
		retest:
			'Stack true-looking candidates, estimate modal bbox and pole transform, and retain onFrac/offFrac.',
		sources: [
			'docs/nuthing-p2/cx-catalog.md: CX-005',
			'src/lib/nuthing/endpoints.ts: matchBasketSprites'
		]
	},
	{
		...
		claim:
			'C1/C2 rings and fills repeat at basket-centered radii and can occlude or imitate corridor/tee evidence.',
		scope:
			'Dev72: C1 solid ring roughly 44±8 px; C2 dashed ring roughly 84±12 px for band scoring.',
		evidence: [
			'Anchor-centered stacks reproduce the repeated zone stamp.',
			'Radial fills defeat ordinary perpendicular-flank tests.'
		],
		use: ['Attribute circular evidence before treating it as a tee or route edge.'],
		doNotInfer: [
			'Furniture proximity is not an endpoint veto.',
			'Band tolerances are not literal stroke thickness.'
		],
		breakers: [
			'Different zoom or scoring-circle display settings.',
			'Neighbor objects at non-repeating offsets.'
		],
	...
	},
	{
		...
		claim:
			'A badge is a white frame around a dark plate containing white digit glyphs; the frame may merge with sprites while the dark plate remains recoverable.',
		scope: 'Dev72 badge family; approximate frame 54×42 and plate 48×36 px.',
		evidence: [
			'Dark-plate recovery restored six sprite-merged badges and reached 72/72 badge availability.'
		],
		use: ['Separate plate detection, glyph decoding, and synthesized frame localization.'],
		doNotInfer: [
			'A white rectangular frame is not necessarily a tee.',
			'Recovered geometry is not an ordinary component measurement.'
		],
		breakers: [
			'Different badge scale/theme.',
			'Sprite intrusion creating phantom digit components.'
		],
	...
	},
	{
		...
		claim:
			'Control-corrected local gray lift separated corridor, walking path, and zone fill families on Dev72.',
		scope:
			'Historical measurements: tee→badge +48, badge→basket +33, walking path +17, C2 fill −7.',
		...
		breakers: [
			'Different imagery/color processing.',
			'Reference windows containing the candidate itself.'
		],
	...
	},
	{
		...
		claim:
			'Visible tees are hollow white rectangular glyphs whose principal axis carries route-direction information.',
		scope: 'Dev72 renderer family; modal size varies by course.',
		evidence: [
			'Ring-tier components preserve enclosing-hole and principal-axis measurements.',
			'Walking-path dashes and badge frames are dominant false-positive families.'
		],
		use: ['Retain bbox, axes, angle, fill, ring fraction, and provenance—not center alone.'],
		doNotInfer: [
			'A rectangle-like component is not automatically a tee.',
			'Component tier does not manufacture enclosing-ring evidence.'
		],
		...
		sources: [
			'docs/nuthing-p2/cx-catalog.md: CX-009',
	...
	},
	{
		...
		claim:
			'The visible tee body aims along its own badge ray, but component PCA is a noisy orientation instrument and does not define a trustworthy hard maximum.',
		scope:
			'Cold rerun of current straight-hole annotations: 40 measurable ring-tier PCA axes; median 1.08°, P90 2.45°.',
		evidence: [
			'Three PCA readings exceeded the historical 2.65° P90: TowneLake H5 11.28°, Heritage H17 6.10°, Lenard H16 2.71°.',
			'Focused pixels show the TowneLake H5 tee and badge ray essentially vertical while PCA is visibly skewed.'
		],
		use: ['Rank badge ownership hypotheses and diagnose flipped tee orientation.'],
		doNotInfer: [
			'The historical 11.3° maximum is a real geometric tail; it reproduced as a PCA contamination artifact.',
			'“Directly” means a tight measured relationship, not exact zero degrees.',
			'Axis direction is 180° ambiguous without badge position.'
		],
		breakers: [
			'Bad PCA geometry after handoff.',
			'A pad whose visible fragment biases its measured axis.'
		],
		retest:
			'Measure absolute acute angle from tee major axis to every badge and preserve alternatives.',
	...
	},
	{
		...
		claim:
			'On the measured Dev72 family, the badge lies on the first corridor segment, so tee→badge estimates departure direction.',
		scope: 'Dev72 historical annotations and renderer measurements.',
		evidence: [
			'Used to seed Z-fit first-segment direction.',
			'Basket→badge failed as dogleg approach truth precisely because the badge precedes the bend.'
		],
		...
		retest:
			'For every visibly bent route, locate the first bend and test badge containment on the tee-side arm.',
	...
	},
	{
		...
		claim:
			'Badge projection along the tee→basket chord measured 0.17–0.54, median 0.51, on 72 Dev holes.',
		...
		use: [
			'Downweight ownership hypotheses placing the badge far behind the tee or beyond the basket.'
		],
		doNotInfer: [
			'The median is not the correct center of a symmetric prior.',
			'Chord fraction does not describe along-route distance on doglegs.'
		],
	...
	},
	{
		...
		detectors: ['D07-straight-ray', 'D08-pair-matrix', 'D09-global-assignment'],
		claim:
			'For known straight holes, tee→badge and tee→basket directions differed by at most 1.4° in Dev72.',
		scope: '41 historical straight holes; median 0.5°, P90 0.9°.',
		evidence: [
			'A bounded collinearity bonus resolved six Lenard cluster thefts without measured regression.'
		],
		use: ['Reward a strongly supported straight hypothesis.'],
		doNotInfer: [
			'Non-collinearity does not prove a bend.',
			'Do not penalize every hole for failing a subset invariant.'
		],
		breakers: [
			'Wrong endpoint ownership.',
			'Parallel neighboring fairways producing a false perfect line.'
		],
	...
	},
	{
		...
		claim:
			'Within the historical cached route extraction, the concatenated true route had zero self-overlap in all 61 measured pairs, while false competitors reached 0.88.',
	...
	},
	{
		...
		claim:
			'In the measured Dev72 corridor family, hole paint approaches the basket anchor from one side and ends in a cap.',
		scope:
			'Dev72 corridor renderer; other overlay families may leave the anchor in other directions.',
		evidence: [
			'Endpoint-cap probes support one-sided corridor geometry.',
			'Naive direction tests suffered 180° traps from walking paths and neighboring corridors.'
		],
		use: ['Test terminal approach candidates and hidden endpoint evidence.'],
		doNotInfer: [
			'All paint leaving the basket is not the hole corridor.',
			'A high backwalk confidence is not correctness.'
		],
		breakers: [
			'Walking trace leaving opposite the approach.',
			'Radial C1/C2 fills and neighboring corridors.'
		],
	...
	},
	{
		...
		claim:
			'A ≤2-bend search with bend angle ≤60°, connector ≤3W, and total length ≤1.4× chord covered every Dev72 hole.',
		...
		doNotInfer: [
			'UDisc cannot render three bends.',
			'The best-scoring Z-fit is final annotation geometry.'
		],
		breakers: [
			'A future three-bend course.',
			'Brightness-biased evidence producing a false bridge.'
		],
	...
	},
	{
		...
		evidence: [
			'Early ring-furniture exclusion deleted these real tees; retaining and tagging onRing restored recall.'
		],
		use: ['Preserve actual onRing evidence as provenance rather than rejecting the candidate.'],
		doNotInfer: [
			'A candidate on a ring is therefore a tee.',
			'Tier may not be substituted for measured onRing.'
		],
	...
	},
	{
		...
		claim:
			'Within the current 18-hole assignment model, each numbered badge owns one tee and one basket, and each candidate endpoint is used once.',
		scope: 'Current ChainSpot solver grammar; not asserted as universal UDisc physics.',
		evidence: [
			'Global uniqueness resolved ownership cases that remained locally below rank 1.',
			'Missing endpoints caused theft chains rather than becoming harmless omissions.'
		],
		use: ['Resolve permutations only after candidate identity and inventory are trustworthy.'],
		doNotInfer: [
			'One-to-one consistency proves the selected permutation.',
			'A complete permutation should be forced when an endpoint is missing.'
		],
		breakers: [
			'Duplicate/missing detector candidates.',
			'Layouts sharing a physical tee or basket outside the current grammar.'
		],
		retest:
			'Surface every endpoint reuse, omission, and displaced ownership rather than only aggregate assignment count.',
		sources: [
			'docs/nuthing-p2/cx-catalog.md: CX-029, CX-042',
			'scripts/nuthing/pair-matrix-replay.ts'
		]
	},
	{
		...
		claim:
			'At Dev72, local true-pair rank 1 saturated at 65/72 while one-to-one assignment was 72/72.',
		scope: 'Exact Dev72 score family at commit 6d7f0fe.',
		evidence: [
			'Five of seven non-rank-1 rows were same-tee/wrong-basket rivals.',
			'Cached tuning attempts were inert or regressed assignment to 63–67.'
		],
		use: [
			'Report candidate rank and final ownership separately; inspect rival-conditioned structure.'
		],
		doNotInfer: [
			'A lower rank means the assignment is wrong.',
			'More local score multipliers will necessarily improve ownership.'
		],
		breakers: [
			'A validation course where true pairs fall beyond the shallow candidate pool.',
			'A changed score family with new independent evidence.'
		],
		retest:
			'Track rank-1, rank≤3, assigned ownership, and theft chains after every algorithm change.',
	...
	},
	{
		...
		claim:
			'When a renderer repeats the same semantic object or graphic primitive many times in one raster, the useful signal is the joint repeatability of several independently measured features—not one universal scalar or one hand-picked exemplar.',
		scope:
			'Generic ChainSpot LAB methodology for repeated renderer families: badges, basket sprites, C1/C2 furniture, tee glyphs, corridor width/paint signatures, and future repeated overlay primitives.',
		evidence: [
		...
		sources: [
			...
			'2026-08-22 held-out validation family studies'
		]
	},
	{
		...
		detectors: ['D04-tee-candidates', 'D05-occluded-tee-recovery'],
		claim:
			'An intact tee is best identified as a course-local renderer family whose white component geometry, enclosed-hole size, and gray interior payload repeat tightly together; no single scalar is the family.',
		scope:
			'Dev72 annotation-graded intact tee rings plus blind held-out validation family measurements from Beaver Ranch Gold, Coleto Creek, Fountain Hills, and SeaTac on 2026-08-22. Validation family membership remains provisional until Oracle truth exists.',
		evidence: [
		...
		retest:
			'On each fresh raster, print the distributions of white-component geometry, enclosed-hole area, gray count/fraction, and neighborhood support. With truth, report real minimum/P50/P100 and maximum false-positive values; without truth, keep family/non-family labels provisional and print the pixels before changing thresholds.',
		sources: [
			'I20 Useful Family Signal',
			'2026-08-22 LAB tee gray/shape study across Dev72 + Beaver/Coleto/Fountain/SeaTac validation',
			'I09 tee-glyph anatomy',
			'C00 Tee Shard case semantics'
		]
	}
] as const;
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
	...
	if (requested === undefined) {
		for (const card of INVARIANT_CARDS)
			console.log(`${card.id} [${card.strength}] — ${card.title}`);
	} else {
		const card = INVARIANT_CARDS.find(
