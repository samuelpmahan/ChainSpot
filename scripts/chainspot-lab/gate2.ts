...
import { matchBasketSpritesSmart, type SmartBasketTemplate } from '../../src/lib/nuthing/smartBasket';
...
function main(): void {
  ...
  console.log(`Gate 2 basket objects: ${baskets.length} = ${clean.length} clean + ${recovered.length} recovered`);
  console.log(`confidence: ${high.length} high, ${medium.length} medium, ${baskets.length - high.length - medium.length} low`);
  console.log('');
  ...
  for (const b of baskets) {
    const color: readonly [number, number, number] =
      b.tier === 'clean-family' ? [45, 225, 80] : b.confidence === 'high' ? [35, 215, 235] : [255, 190, 40];
    ...
    JSON.stringify(
      {
        ...
        canonical: { width: image.width, height: image.height, rows: [viewport.top, viewport.bottom] },
        badgeCount: stage.badgeCount,
        counts: { total: baskets.length, clean: clean.length, recovered: recovered.length, high: high.length, medium: medium.length },
        baskets,
      },
      null,
      2,
