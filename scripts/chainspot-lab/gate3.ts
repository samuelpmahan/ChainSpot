...
function circle(png: PNG, cx: number, cy: number, r: number, rgb: [number, number, number]): void {
  for (let a = 0; a < 360; a++) {
    ...
    for (let w = 0; w < 3; w++) {
      setPixel(png, Math.round(cx + (r + w) * Math.cos(t)), Math.round(cy + (r + w) * Math.sin(t)), rgb);
    }
  }
}
...
function main(): void {
  ...
  console.log(`Gate 0 canonical: ${full.width}x${full.height} -> ${image.width}x${image.height}; rows [${viewport.top},${viewport.bottom})`);
  ...
  console.log(`Gate 2 basket candidates: ${sprites.length}; strong(score>=0.8)=${sprites.filter((s) => s.score >= 0.8).length}`);
  ...
  console.log(`basket outline touch median=${s.basketOutlineTouchMedian.toFixed(1)}; component minimum=ceil(0.2*median)=${s.componentDarkTouchMin}`);
  ...
  console.log(`post-touch removals: nearRing=${s.removedNearRing} nearDiamond=${s.removedNearDiamond} nearSprite=${s.removedNearSprite}`);
  console.log(`component accepted=${s.componentAccepted}`);
  console.log(`FINAL TEE CANDIDATES=${s.totalAccepted} (${s.ringTeeRects} ring + ${s.componentAccepted} component)`);
  ...
  for (const p of measured.points) {
    circle(png, Math.round(p.cx), Math.round(p.cy), 10, p.tier === 'ring' ? [50, 235, 255] : [255, 70, 220]);
  }
