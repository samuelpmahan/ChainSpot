...
production build clean; focused detector/zero-bend suites green (19 tests,
including ambiguity and degenerate-chord cases); no import of
`scripts/toph/record` reachable from `src/`. Fresh reproduction corrected the
source experiment's zero-bend headline: the active joint predicate resolves
43/44 straight truth-geometry cases and abstains on one, with 0/28 bent
candidates and no ambiguity. A 4px comparison reaches 44/44 in that proxy but
does not change DashsTrack's exact production assignments, so the safer 3px
default remains. This evidence is N=4 and deliberately excludes AlexClark.
E2E/browser tests skipped per AGENTS.md (no UI surface changed).
