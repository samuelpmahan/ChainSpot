...
## Post-implementation addendum (2026-08-14): the Annotate Round route split

§3's verdict on "Annotate Round route" — **KEEP**, "Unchanged" — has been superseded. The route was itself two activities wearing one costume: Map mode (course geometry) and Round mode (a played round's throws/walk path) behind an internal, easy-to-miss toggle in the shared app header. It is now two real routes, `/annotate-course` and `/annotate-round`'s Round half → `/map-round`, sharing one implementation (`src/lib/components/AnnotationWorkspace.svelte`) parameterized by a `mode` prop fixed once by whichever thin route mounts it, rather than a runtime-switchable `$state`. `src/lib/annotationNav.svelte.ts` lost its mode field entirely (only the shared "Done" request/callback remains); `src/lib/session.ts`'s `EditorSessionKey` split `'annotate-round'` into `'annotate-course'` and `'map-round'`, each an independent retained-editor slot; `PendingHandoff` gained a `destination` field to disambiguate which of the two now-separate routes claims a `source-overview`-role image, since both accept one.

The practical consequence worth knowing: Annotate Course and Map Round no longer share an in-memory editor. A user who annotates a course and wants to map a round on it in the same sitting now goes through Course Memory recognition (save on Annotate Course, import/recognize on Map Round) rather than an invisible mode switch — the same mechanism `/demo`'s reload step already showcased, now the *only* path rather than one of two. `docs/demo-walkthrough.md` and `docs/course-memory.md` were updated for the split; `docs/13-inch-pass.md` was not re-measured against it (see its own note). A pre-existing gap the split's own test-fixing surfaced, not caused by it: several e2e specs (`holeAnnotation.spec.ts`, `holeGraphics.spec.ts`, `holePersistence.spec.ts`, `radialMenu.spec.ts`) predate an earlier sidebar-driven-placing-flow redesign and an off-by-default "enable radial menu" dev toggle, and need real behavioral rework, not a mechanical rename, before they pass again.
