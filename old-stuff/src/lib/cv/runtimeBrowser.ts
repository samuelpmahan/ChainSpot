 ...
 * ever reached via a dynamic `import('./runtimeBrowser')` from the
 * browser/worker branch of `runtime.ts`'s `getCv()`. See that file's doc
 * comment for why the static import matters and why this can't just live
 * inline in `runtime.ts`: a static import at that module's top level would
