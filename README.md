...
The active Spot Round project also survives client-side navigation: `src/lib/spotRoundSession.ts` retains the live `ProjectEditor` in memory across `/spot-round` ↔ `/stitch-map` moves, so loaded images, the project name, control points, undo/redo history, dirty state, and decoded image resources persist until a full page reload.
