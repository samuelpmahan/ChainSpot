...
The active editor for each stage also survives client-side navigation: `src/lib/session.ts` retains the live `ProjectEditor` in memory per stage across `/annotate-round` ↔ `/create-graphics` ↔ `/stitch-map` moves, so loaded images, the project name, control points, undo/redo history, dirty state, and decoded image resources persist until a full page reload.
