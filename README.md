...
`/demo` is a shareable guided tour for someone seeing ChainSpot for the first time. It is a narration layer over the real product, never a second implementation of it: each step loads real UDisc captures of a real course, hands them to the same intake paths the app's own file pickers use, and navigates to the real `/stitch-map`, `/annotate-course`, `/map-round`, and `/create-graphics` routes. A guide rail mounted in the app layout carries the narration across those navigations and renders nothing when no tour is running.

The six-step script builds a course map once (stitch, then annotate its geometry on Annotate Course), puts it on air (a live basemap fetch plus export), then proves the map is reusable: a real browser reload clears everything in-session, and the walkthrough finishes by annotating a *played round* of the same course (Map Round: throws and the walking path) purely from what Course Memory remembered across that reload, before exporting again. The clean basemap is deliberately never shipped as a fixture — the basemap/export steps fetch it live, both before and after the reload, from the same OpenStreetMap Nominatim and USGS NAIP endpoints the product already uses. Exiting the tour leaves the loaded project fully usable, so a visitor can carry straight on with their own course. Design notes and the rules the demo holds itself to are in `docs/demo-walkthrough.md`; the dataset's provenance is in `static/resources/demo/dashs-track/README.md`.

## Stitch Map page

`/stitch-map` combines four higher-zoom screenshots of one map into a single higher-detail image before either downstream stage: `/annotate-course` (UDisc source capture; `/` redirects there) or `/create-graphics` (the correspondence workflow). The workflow is intentionally a controlled 2×2 screenshot compositor:

- **Capture protocol**: four screenshots at one fixed zoom/orientation, captured upper-left → upper-right → lower-left → lower-right, all at the same device/screenshot size. Neighbors need at least roughly 20% overlap; more is always fine and matches more strongly, so a generous overlap is never treated as a problem. The four must be four *different* captures: a bulk import containing the same screenshot twice is rejected by name rather than arranged into a map that silently omits a quarter of the course. The first valid tile establishes the required dimensions; every other tile must match, and the requirement resets only when all four slots are empty. Each slot is protected against stale in-flight decodes: a newer selection, removal, or reset invalidates earlier decodes for that slot.
...
- **Handoff to Annotate Course / Create Graphics**: **Use as UDisc source** renders the PNG, holds it in an in-memory pending handoff, and navigates to `/annotate-course`; **Use as clean target** does the same and navigates to `/create-graphics`. Each destination shows a banner offering **Import** (through the normal image-intake and replacement rules, including point-discard confirmation, undo/redo, and dirty state, where applicable) or **Dismiss**. A pending handoff is never silently overwritten, and it survives round trips between the two pages until imported or dismissed.

The active editor for each stage also survives client-side navigation: `src/lib/session.ts` retains the live `ProjectEditor` in memory per stage across `/annotate-course` ↔ `/map-round` ↔ `/create-graphics` ↔ `/stitch-map` moves (Annotate Course and Map Round each keep their own independent editor, never sharing one), so loaded images, the project name, control points, undo/redo history, dirty state, and decoded image resources persist until a full page reload.
